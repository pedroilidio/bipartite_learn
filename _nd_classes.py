...
def duplicate_if_needed(something):
    if type(something) not in {list, tuple}:
...
class BaseDecisionTree2D(BaseDecisionTree, metaclass=ABCMeta):
    ...
    def fit(self, X, y, sample_weight=None, check_input=True):
        ...
        if check_input:
            raise NotImplementedError("We do not check inputs here.")
        ...
        n_samples, self.n_features_in_ = y.size, sum(Xax.shape[1] for Xax in X)
        ...
        # self.n_outputs_ = ...  # TODO: implement.
        self.n_outputs_ = 1
        ...
        if not isinstance(criterion, Criterion):
            if is_classification:
                ...
                criterion_rows = CRITERIA_REG[self.criterion](
                    n_outputs=self.n_outputs_,
                    n_samples=y.shape[0],
                )
                criterion_cols = CRITERIA_REG[self.criterion](
                    n_outputs=self.n_outputs_,
                    n_samples=y.shape[1],
                )
            ...
            criterion_rows = copy.deepcopy(criterion_rows)
            criterion_cols = copy.deepcopy(criterion_cols)

        SPLITTERS = SPARSE_SPLITTERS if issparse(X) else DENSE_SPLITTERS

        splitter = self.splitter
        if not isinstance(self.splitter, Splitter2D):
            # TODO: define Splitter2D builder function.
            splitter_rows = SPLITTERS[self.splitter](
                criterion_rows,
            ...
            splitter_cols = SPLITTERS[self.splitter](
                criterion_cols,
